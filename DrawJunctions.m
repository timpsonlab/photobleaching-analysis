function DrawJunctions(frap, path)
...
%       2. Press 'd'
    ...
    % Draw image to select junctions
    if isstruct(frap)
        % Draw first frame of FRAP stack after bleach
        im = frap.after{1};        
        kern = fspecial('disk',5);
        im =  conv2(double(im),kern,'same');
        imagesc(im);
        hold on;
        plot(frap.roi_x,frap.roi_y,'r');
        hold off;
    else
        imagesc(frap)
    end
