...
% pair is the nth pair of a lag time
       ...
       RawGt = zeros(nanmean(1:size(imgser,3))*(size(imgser,3)),2) - 1;
       GtAvg = zeros(size(imgser,3),2);
       %imgser = double(imgser);
       index = 1;
       %Calculates mean of each image before to save time in calculation
       imgSerMean = squeeze(nanmean(nanmean(imgser,2)));
       
       imgser = imgser - nanmean(imgser(:));
       ...
       for tau = 0:(size(imgser,3)-1)
           ...
           for pair=1:(size(imgser,3)-tau)
               ...
               lagcorr(1,1,pair) = nanmean(corrNonAvg(:));
           ...
           GtAvg(tau+1,2) = squeeze(nanmean(nanmean(lagcorr)));
